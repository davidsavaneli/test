...
`null`); **`multiple`** → an array. A dropzone (click-to-browse + OS drag-drop) sits above a responsive grid
of cards (≥200px wide, stretching to fill the row and wrapping when there are many). An image shows a
thumbnail, a **video** a first-frame preview with a centered play badge, other files a file icon; each card
overlays the name + size, a remove ×, download, a **view (Eye) → fullscreen lightbox** (image + video only),
and — for images — crop + alt-text buttons. Reorder by drag or keyboard. Props: `multiple` · `allowDrop` ·
`allowReorder` · `allowDownload` · `allowPreview` ·
...
(`autocomplete="one-time-code"`) fill the whole code across the boxes. Value accepts — and `onChange` /
`onComplete` mirror — **either a joined `string` or a per-box `string[]`** (`['1','2','3','4']`);
...
`<Form>` by `name` (value = the code `string` or `string[]`; validate with `z.string().length(4)` or
...
**`steps`** (`{ label?, description?, optional?, icon?, completed?, error?, disabled?, content? }[]`),
controlled (**`activeStep`**, **0-based**) or uncontrolled (**`defaultStep`**): steps before it show
**completed** (check), the one at it is **active** (halo ring), the rest **upcoming** (number). URL sync
is **opt-in (off by default)**: pass **`queryKey`** (bare = the param name from
`config.keys.stepQueryKey`, else `'step'`; or a custom string) to sync the active step (1-based
`?step=2`) so a refresh restores it — `popstate` fires **`onStepChange(index)`** (wire it to your state
in controlled mode); `pushHistory` makes Back walk the steps. Two layouts: `horizontal` (default — labels
...
`<Table onExportToEmail={(s) => exportCustomers(s.query)} data={rows} columns={cols} />`
