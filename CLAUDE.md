...
`3`, the starting + smallest height) up to an optional **`maxRows`** (then it caps and scrolls — the
auto-grow logic flips `overflow-y` to `auto`). It does **not** carry the input-only extras (adornment /
